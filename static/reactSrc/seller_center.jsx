    ...
    return(
        <div className='sidebar-menu-item mb-5 !flex-col'>
            <div className={`flex w-[100%]`} onClick={()=>setOpen(!isOpen)}>
                ...
                {
                    (subtitles.length === 0)?
                        <a  className='sidebar-menu-item-text grow' href={href}>{title}</a>:
                        <span className='sidebar-menu-item-text grow' >{title}</span>
                }
                {(subtitles.length !== 0) && (
                    <i className={`sidebar-menu-item-icon shopee-icon sidebar-menu-item-collapse`}
            ...
            <ul className={`sidebar-submenu`}>
                {
                    ...
                    subtitles.map(i=>(
                        <li className={`sidebar-submenu-item`}>
                            <a className={`sidebar-submenu-item-link`} href={i.href}>{i.title}</a>
                        </li>
                    ))
                }
            </ul>
    ...
    return (
        <div className='sidebar-container'>
            <ul className='sidebar-menu'>
                {
                    titles.map(title=>(
                        <Section title={title.title} href={title.href} subtitles={title.subtitles}></Section>
                    ))
                }
            </ul>
        </div>
