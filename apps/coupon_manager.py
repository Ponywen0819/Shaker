...
def get_coupon():
    ...
    coupons = db.command_excute("""
        ...
        WHERE (publisher_id = %(publisher_id)s OR discount_type = 0) AND end_time > %(time)s
...
@app.route("/GetAdminCoupons", methods=["POST"])
def get_admin_coupon():
    ...
    coupons = db.command_excute("""
        ...
        FROM coupon
        JOIN coupon_type ON coupon.type = coupon_type.id
        WHERE publisher_id = %(publisher_id)s AND end_time > %(time)s
    """, {
        "publisher_id": admin_info[0]['publisher_id'],
